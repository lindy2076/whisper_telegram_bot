...
async def help_handler(message: types.Message):
    """sends help"""
    await message.answer(
        Response.HELP, reply_markup=start_kb, parse_mode=ParseMode.HTML
    )
...
async def video_handler(message: types.Message):
    await message.reply("this is a video msg!")
...
async def video_note_handler(message: types.Message, bot: Bot):
    ...
    await bot.download(file=file_id, destination=tmp_filename)
    ms = await message.reply("video downloaded!")
...
@main_router.message()
async def non_voice_handler(message: types.Message):
...
async def whisper_callback_handler(
    ...
    await query.message.edit_text(
        Response.select_curr_model(model.mdl),
...
async def fmt_cleanup_callback_handler(
    ...
    if try_remove(f"tmp/{transcript_id}.txt"):
        ...
        q = await query.message.answer(
            f"{transcript_id} removed", parse_mode=ParseMode.HTML
        )
